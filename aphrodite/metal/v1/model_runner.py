...
from aphrodite.v1.outputs import LogprobsTensors, ModelRunnerOutput
from aphrodite.v1.sample.logits_processor import build_logitsprocs
from aphrodite.v1.sample.metadata import SamplingMetadata
from aphrodite.v1.sample.sampler import Sampler

from aphrodite.metal.config import get_config
from aphrodite.metal.paged_attention_backend.hybrid import HybridPagedAttentionBackend
...
from aphrodite.metal.paged_attention_common import (
    OffsetCache,
    clear_context,
    prepare_unified,
)
from aphrodite.metal.stt.runtime import STTRuntimeAdapter
from aphrodite.metal.stt.serve import AphroditeSTTRequestAdapter
from aphrodite.metal.v1 import contiguous_cache
from aphrodite.metal.v1.cache_policy import ModelCachePolicy
from aphrodite.metal.v1.contiguous_cache import (
...
from aphrodite.metal.v1.model_lifecycle import ModelLifecycle
from aphrodite.metal.v1.sampling_batch import (
    ...
    _merge_single_row_logprobs,
)
from aphrodite.metal.v1.structured_output import MetalStructuredOutputApplier
...
class _ExecutionBatch:
    ...
    logprobs_tensors: list[LogprobsTensors | None] = field(default_factory=list)
    ...
    def add_output(
        ...
        self.logprobs_tensors.append(logprobs_tensors)
        return output_idx
...
class _NonPagedPrefetch(NamedTuple):
    """One-token lookahead state for the contiguous KV path."""
...
def _make_top_k_first_mlx_sampler(
    *,
    temperature: float,
    top_p: float,
    top_k: int,
    ...
    """Build an MLX sampler matching Aphrodite's common sampler order."""
    if temperature < 1e-5:
        return lambda logits: mx.argmax(logits, axis=-1)

    use_top_k = 0 < top_k < vocab_size
    use_top_p = top_p < 1.0

    def sampler(logprobs: mx.array) -> mx.array:
        logits = logprobs * (1.0 / temperature)

        if use_top_k:
            topk_indices = mx.argpartition(-logits, top_k - 1, axis=-1)[
                :, :top_k
            ]
            ...
            if use_top_p:
                ...
                remove = (mx.cumsum(sorted_probs, axis=-1) - sorted_probs) > top_p
...
def _slice_logprobs_row(
    ...
    return LogprobsTensors(
        logprobs_tensors.logprob_token_ids[index : index + 1],
        logprobs_tensors.logprobs[index : index + 1],
        logprobs_tensors.selected_token_ranks[index : index + 1],
    )
...
class MetalModelRunner:
    ...
    def __init__(
        ...
        custom_lp = aphrodite_config.model_config.logits_processors
        ...
        self._pending_mlx_generators: dict[str, Any] = {}
        ...
        self._non_paged_prefetch: _NonPagedPrefetch | None = None
    ...
    def _forward_model_logits(
        ...
        input_ids: mx.array,
        *,
        cache: list[OffsetCache],
        logit_indices: list[int] | None = None,
    ) -> mx.array:
        """Run the model and optionally project only selected token rows.

        During paged prefill the transformer must process every prompt token
        to populate KV cache, but serving only samples from decode rows and
        the final row of each prefill segment.  Avoid applying the expensive
        vocab projection to prompt rows that will never be consumed.
        """
        model = self._forward_model
        if logit_indices is None or not hasattr(model, "model"):
            return self._extract_logits(model(input_ids, cache=cache))

        hidden = model.model(input_ids, cache=cache)
        if logit_indices:
            hidden = hidden[:, mx.array(logit_indices, dtype=mx.int32), :]
    ...
    @staticmethod
    def _can_compact_paged_logits(
        scheduler_output: SchedulerOutput,
        prefill_reqs: list[PrefillRequest],
    ) -> bool:
        """Return whether only sample-position logits are needed."""
        if scheduler_output.has_structured_output_requests:
            return False
        return all(pr.sampling_params.prompt_logprobs is None for pr in prefill_reqs)
    ...
    def _prefill_single(
        ...
        sampling_params: SamplingParams,
        generator: torch.Generator | None = None,
    ) -> tuple[int, list[KVCache], LogprobsTensors | None]:
        ...
        cache: list[KVCache]
        cached_prefix_len = 0
        fast_generator = self._make_mlx_lm_generator(token_ids, sampling_params)
        if fast_generator is not None:
            generator_iter, cache = fast_generator
            next_token, _logprobs = next(generator_iter)
            self._pending_mlx_generators[req_id] = generator_iter
            return int(next_token), cache, None
        ...
        sample_result = sample_from_logits(
            last_logits, batch, self._sampler, self.device
        )
        [next_token] = sample_result.token_ids
        mx.eval(*[c.state for c in cache])

        return next_token, cache, sample_result.logprobs_tensors
    ...
    def _batched_decode(
        ...
        next_tokens = sample_result.token_ids
    ...
    def _sequential_decode(
        ...
        """
        next_tokens = []
        logprobs_tensors: list[LogprobsTensors | None] = []

        for req_id, state in decode_reqs:
            if len(decode_reqs) == 1 and state.mlx_generator is not None:
                next_token, _logprobs = next(state.mlx_generator)
                next_token = int(next_token)
                state.token_ids.append(next_token)
                state.generated_tokens += 1
                next_tokens.append(next_token)
                logprobs_tensors.append(None)
                continue

            last_token = state.token_ids[-1] if state.token_ids else 0
            prefetch = self._non_paged_prefetch
            if (
                len(decode_reqs) == 1
                and prefetch is not None
                and prefetch.req_id == req_id
                and prefetch.token_id == last_token
            ):
                last_logits = prefetch.logits
                self._non_paged_prefetch = None
            else:
                self._non_paged_prefetch = None
            ...
            next_tokens.append(next_token)
            logprobs_tensors.append(sample_result.logprobs_tensors)

            # Update state
            state.token_ids.append(next_token)
            state.generated_tokens += 1
            self._maybe_start_non_paged_prefetch(req_id, state, next_token)

        return MetalSamplerResult(
            next_tokens,
            _merge_single_row_logprobs(logprobs_tensors),
        )

    def _maybe_start_non_paged_prefetch(
        self,
        req_id: str,
        state: RequestState,
        token_id: int,
    ) -> None:
        """Submit one-token lookahead for raw MLX-style decode pipelining."""
        if (
            self._paged_attention_backend is not None
            or self._prefix_cache is not None
            or len(self._request_states) > 1
        ):
            self._non_paged_prefetch = None
            return
        ...
        model_output = self._forward_model(input_ids, cache=state.cache)
        logits = self._extract_logits(model_output)[:, -1, :]
        mx.async_eval(logits)
        self._non_paged_prefetch = _NonPagedPrefetch(
            req_id=req_id,
            token_id=token_id,
            logits=logits,
        )
    ...
    def _start_paged_forward(
        ...
        if self.is_hybrid:
            from aphrodite.metal.paged_attention_common import get_context
        ...
        logit_cu_seqlens = cu_seqlens
        logit_indices: list[int] | None = None
        if prefill_reqs and self._can_compact_paged_logits(
            scheduler_output, prefill_reqs
        ):
            logit_indices = list(range(num_decode))
            logit_indices.extend(
                cu_seqlens[num_decode + j + 1] - 1
                for j in range(len(prefill_reqs))
            )
            if len(logit_indices) < len(all_token_ids):
                logit_cu_seqlens = list(range(len(logit_indices) + 1))
            else:
                logit_indices = None
        ...
        try:
            ...
            # MLX uses lazy evaluation — logits holds the computation graph.
            # Evaluating only sample-position logits lets MLX free intermediate
            # buffers without materializing a full prompt × vocab plane.
        finally:
            clear_context()
    ...
    def _sample_paged_batch(
        ...
        vocab_size = self._vocab_size
        logitsprocs = self._logitsprocs
        decode_sample_result = sample_decode_tokens(
            logits,
            decode_reqs,
            num_decode,
            self._sampler,
        ...
        decode_next_tokens = decode_sample_result.token_ids
        prefill_sample_result = sample_prefill_tokens(
            logits,
            prefill_reqs,
            cu_seqlens,
            num_decode,
        ...
        prefill_next_tokens = prefill_sample_result.token_ids
        ...
        for i, entry in enumerate(batch.paged_prefill_entries):
            ...
            prefill_logprobs = _slice_logprobs_row(
                prefill_sample_result.logprobs_tensors, i
            )

            if entry.result_mode == "intermediate":
                batch.sampled_tokens[entry.output_idx] = []
                batch.logprobs_tensors[entry.output_idx] = None
                continue

            batch.sampled_tokens[entry.output_idx] = [next_token]
            batch.logprobs_tensors[entry.output_idx] = prefill_logprobs
        ...
        for i, (req_id, _) in enumerate(batch.paged_decode_reqs):
            batch.add_output(
                req_id,
                [decode_next_tokens[i]],
                _slice_logprobs_row(decode_sample_result.logprobs_tensors, i),
            )
    ...
    def _handle_new_requests(
        ...
        for new_req in new_reqs:
            ...
            next_token, cache, logprobs_tensors = self._prefill_single(
                req_id,
                token_ids,
                sampling_params,
                generator=generator,
            ...
            if self._request_states[req_id].mlx_generator is None:
                self._maybe_start_non_paged_prefetch(
                    req_id,
    ...
    def _build_output(batch: _ExecutionBatch) -> ModelRunnerOutput:
        ...
        logprobs_tensors = _merge_single_row_logprobs(batch.logprobs_tensors)
        return ModelRunnerOutput(
            req_ids=batch.req_ids,
            req_id_to_index=batch.req_id_to_index,
            sampled_token_ids=batch.sampled_tokens,
            logprobs=(
                None if logprobs_tensors is None else logprobs_tensors.tolists()
            ),
    ...
    def _run_non_paged_decode_batch(
        ...
        if batch.valid_decode_reqs:
            ...
            for i, (req_id, _) in enumerate(batch.valid_decode_reqs):
                batch.add_output(
                    req_id,
                    [decode_result.token_ids[i]],
                    _slice_logprobs_row(decode_result.logprobs_tensors, i),
                )
    ...
    def _cleanup_finished_requests(
        ...
        for req_id in finished_req_ids:
            self._pending_mlx_generators.pop(req_id, None)
            if (
                self._non_paged_prefetch is not None
                and self._non_paged_prefetch.req_id == req_id
            ):
                self._non_paged_prefetch = None
    ...
    def execute_model(
        ...
        if (
            ...
            raise NotImplementedError(
                ...
                "Enable paged attention (APHRODITE_METAL_USE_PAGED_ATTENTION=1) "
    ...
    def _execute_stt(
        ...
        for new_req in scheduler_output.scheduled_new_reqs:
            stt_request = AphroditeSTTRequestAdapter.from_aphrodite_request(new_req)
            sampling_params = new_req.sampling_params or SamplingParams()
